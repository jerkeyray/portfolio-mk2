...
export default function Home() {
  ...
  return (
    <div className="relative max-w-3xl mx-auto flex flex-col justify-center flex-1 min-h-[calc(100svh-7rem)] md:min-h-0 pb-6 md:pb-10 px-2 md:px-8">
      <section className="space-y-4 md:space-y-7">
        ...
        <div
          className="animate-stagger flex flex-wrap items-center gap-1.5 md:gap-2 text-[10px] md:text-sm font-medium"
          style={{ animationDelay: "100ms" }}
        >
          {["go", "distributed systems", "ai infra", "db internals"].map((tag, i) => (
            <span key={tag} className="flex items-center gap-2">
              {i > 0 && <span className="text-muted-foreground/30">•</span>}
              <span className="bg-accent/10 px-1.5 py-0.5 md:px-2.5 md:py-1 rounded border border-accent/20 text-accent/90 transition-all duration-200 hover:bg-accent/20 hover:border-accent/40 cursor-default">
                {tag}
              </span>
            </span>
          ))}
        ...
        >
          <p className="text-xs md:text-lg text-muted-foreground/90 leading-relaxed">
        ...
        <div
          className="animate-stagger space-y-2 md:space-y-2.5 text-xs md:text-lg"
      ...
      <div
        className="animate-stagger flex items-center gap-4 md:gap-5 pt-6 md:pt-10"
      ...
      >
        {socials.map((social) => {
          ...
          return (
            ...
            >
              <Icon className="size-5 md:size-[25px]" />
            </a>
          );
        })}
