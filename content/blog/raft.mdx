...
In this blog we'll be going over why consensus is necessary, how Raft approaches this problem and all the things that go into making distributed systems agree. This is a no-code language-agnostic walkthrough and focuses more on the why than the how.
...
**Validity** - The decided value must actually be proposed by a node (i.e the system cannot just make up values).
...
**Availability** - Each request receives a response, even if it's not the latest.
...
**Raft chooses Consistency over Availability.**

When the system can not guarantee a consistent view of state, it refuses to proceed. It will stop accepting writes rather than risk diverging histories.
...
While it's mathematically proven and brilliant, Paxos is notoriously difficult to understand even harder to implement.

If an algorithm is too complex for engineers to reason about, they will implement it with bugs. Raft achieves the same performance and safety as Paxos but is easier for people to reason about. 
...
Time in Raft is not measured in seconds, but in leadership epochs. 

A term is a monotonically increasing number that acts a clock for the cluster.
...
- Votes for itself.
- Requests vote from other nodes.
...
If a candidate receives the majority of votes (quorum), it becomes the leader and starts sending heartbeats to the other nodes to assert dominance.
...
4) Once a majority have acknowledged the entry, it is considered committed.

If two nodes contain entry with the same index and term, then all the logs are identical in all entries before that point.
...
Clients always send their requests to the leader. If a client contacts a follower instead, the request will be redirected to leader or outright rejected itself.
...
- duplicate requests can be safely ignored.
...
Distributed systems aren’t mystical or magical, they’re just unforgiving. Most of the complexity comes from dealing with failure, uncertainty, and imperfect information. Once you accept that, the ideas start to feel surprisingly grounded and mechanical.
